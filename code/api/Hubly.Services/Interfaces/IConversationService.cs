...
namespace Hubly.api.Services.Interfaces
{
    ...
    {
        Task<OneOf<int, ConversationError>> CreateConversation(int currentUserId, int? coWorkerId, int? senderCompanyId, int? senderSocialProfileId, int? receiverCompanyId, int? receiverSocialProfileId);
        ...
        Task<OneOf<int, ConversationError>> SendMessage(int currentUserId, int? coWorkerId, int conversationId, string content);
        Task<OneOf<bool, ConversationError>> EditMessage(int currentUserId, int? coWorkerId, int messageId, string newContent);
        Task<OneOf<bool, ConversationError>> DeleteMessage(int currentUserId, int? coWorkerId, int messageId);
