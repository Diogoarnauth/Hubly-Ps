...
import usersService, { type AuditLogEntry } from '@/services/api/UsersService';
...
export function InvitePage() {
    ...
    try {
      ...
      if (isOwner) {
        const [sent, team, audit] = await Promise.all([
          ...
          usersService.getAuditLogs(auditPage, auditPageSize)
        ]);

        setSentInvites(sent);
        setTeamMembers(team);
        setAuditLogs(audit?.items ?? []);
        setAuditTotalItems(audit?.totalItems ?? 0);
      } else if (isCoWorker) {
  ...
  }, [auditPage, auditPageSize, isCoWorker, isOwner, user?.role]);

  const loadAuditLogsPage = useCallback(async (page: number) => {
    if (!isOwner) return;

    setAuditLoading(true);
    setAuditPage(page);

    const response = await usersService.getAuditLogs(page, auditPageSize);
    if (response) {
      setAuditLogs(response.items ?? []);
      setAuditTotalItems(response.totalItems ?? 0);
    } else {
      setAuditLogs([]);
      setAuditTotalItems(0);
    }
        ...
        <CardContent className="p-6">
          ...
          {isOwner && (
            <div className="space-y-8">
              ...
              <Card className="border-zinc-800 bg-[#1F1F1F] text-white rounded-[25px]">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-[#A78BFA]" /> Audit Logs
                  </CardTitle>
                  <CardDescription className="text-zinc-400">
                    Latest workspace activity recorded by the platform.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  {auditTotalItems > auditPageSize && (
                    <div className="flex items-center justify-between border-t border-zinc-800 pt-4">
                      <Button
                        variant="ghost"
                        onClick={() => loadAuditLogsPage(auditPage - 1)}
